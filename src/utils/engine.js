...
SearchController.nodes = null;
SearchController.fh = null;
SearchController.fhf = null;
SearchController.depth = null;
SearchController.time = null;
SearchController.start = null;
SearchController.stop = null;
SearchController.best = null;
SearchController.thinking = null;
...
function AddGUIPiece(sq, pce) {
	...
	var pieceFileName = "https://chess-pieces.s3.us-east-2.amazonaws.com/" + SideChar[PieceCol[pce]] + PceChar[pce].toUpperCase() + ".png";
...
export function makeJSXMove(seconds) {
	GameController.PlayerSide = GameController.side ^ 1;
	if (GameController.GameOver === BOOL.FALSE) {
		SearchController.thinking = BOOL.TRUE;
		...
		SearchController.time = parseInt(seconds) * 1000;
		SearchPosition();
...
export function takeJSXMoveBack(){
	if(GameBoard.hisPly > 0) {
		TakeMove();
		GameBoard.ply = 0;
		SetInitialBoardPieces();
